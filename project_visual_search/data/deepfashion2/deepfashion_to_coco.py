...
from source.utils import setup_google_credentials
from tqdm import tqdm


def generate_json(DATA_PATH, gcs=True):
    ANNO_FOLDER_PATH = DATA_PATH + "annos/"
    IMG_FOLDER_PATH = DATA_PATH + "images/"

    nb_imgs = len(os.listdir(IMG_FOLDER_PATH))
    ...
    for num in tqdm(range(1, nb_imgs + 1), total=nb_imgs):
        json_name = ANNO_FOLDER_PATH + str(num).zfill(6) + ".json"
        image_name = IMG_FOLDER_PATH + str(num).zfill(6) + ".jpg"
    ...
    if gcs:
        ...
        print("Setting up the blob in the bucket")
        blob = bucket.blob(DATA_PATH + "instances.json")
        print("Uploading the json in the bucket")
        blob.upload_from_string(dataset)

    else:
        json_name = DATA_PATH + "instances.json"
